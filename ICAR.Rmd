...
# NOT MY ORIGINAL CODE, USING [THIS REPO](https://github.com/ConnorDonegan/Stan-IAR) FROM Connor Donegan, WITH MODIFICATIONS FOR USE IN THIS PROJECT.

# Flexible Functions for ICAR, BYM, and BYM2 Models in Stan

This repository contains R and Stan code to fit spatial models using intrinsic conditional autoregressive (ICAR) priors, including the BYM model [@besag_1991] and Riebler et al.'s [-@riebler_2016] adjustmented ("BYM2") specification. The code here follows all of the recommendations from @freni_2018 for disconnected graph structures, building on @morris_2019. For proper CAR models in Stan, see [here](https://github.com/ConnorDonegan/survey-HBM).
...
- `icar-functions.stan` Stan functions for calculating log probability of the ICAR prior, and functions to create the BYM(2) convolution term;
- `icar-functions.R` R functions to prepare data for the ICAR Stan models;
- `BYM.stan` Example code for the BYM model. Corresponding R code is provided in this README.md file;
- `BYM2.stan` Example code for the BYM2 model;
- `demo-BYM2.R` Example R code for fitting the BYM2 model.
- `BYM2-b.stan` Alternative code for the ICAR/BYM2 model in Stan (courtesy of Mizi Morris--this is currently under revision, please use `BYM2.stan` for now). 

The foundation for the efficient Stan code for ICAR models was first presented in Morris' Stan [case study](https://github.com/stan-dev/example-models/blob/master/knitr/car-iar-poisson/nb_data_funs.R) and @morris_2019. I've adapted the model to handle a variety of common scenarios, particularly the presence of observations with zero neighbors or data from multiple regions that are disconnected from each other. These situations impact how the sum-to-zero constraint is imposed on the model as well as how the terms of the BYM model can be combined. I drew on previous work by [Adam Howes](https://athowes.github.io/2020/11/10/fast-disconnected-car/) (including his use of the segment function), and the repository includes additional contributions from M. Morris (where indicated, carrying over some material from [Stan-Dev](https://github.com/stan-dev/example-models/tree/master/knitr/car-iar-poisson/update_2021_02)). 

For a general introduction to ICAR models (including spatio-temporal specifications) see @haining_2020. For an introduction to their implementation in Stan see Morris' [Stan case study](https://mc-stan.org/users/documentation/case-studies/icar_stan.html). For a deeper read see @rue_2005.

For fast and stable proper CAR models in Stan, which naturally avoid most of the complications
dealt with here, see @donegan_2021 ([with code](https://doi.org/10.31219/osf.io/3ey65)) or this
[git repo](https://github.com/ConnorDonegan/survey-HBM) from a related project. 
These and other spatial Stan models are implemented in the [geostan](https://connordonegan.github.io/geostan/)
R package; the package also contains convenience functions for fitting custom 
spatial models in Stan (see the `prep_icar_data` and `prep_car_data` functions).
...
The BYM model includes the parameter vector assigned the ICAR prior plus a vector `theta` assigned a normal prior with unknown scale: $\theta \sim N(0, \eta)$, with $\eta$ assigned some prior such as $\eta \sim N(0, 1)$. Again, in practice we assign `theta_tilde` a standard normal prior and then multiply it by its scale `theta_scale`. Then the convolution term is 
```{r eval = FALSE} 
...
This section demonstrates how to fit these models using the Metro data. It requires some functions from `icar-functions.R` in this repository, plus the **spdep**, **sf**, **rstan**, and **ggplot2** packages.

```{r}
pkgs <- c("rstan", "sf", "spdep", "ggplot2")
lapply(pkgs, require, character.only = TRUE)
rstan_options(auto_write = TRUE)
source("Stan-IAR-main/icar-functions.R")
```
...
Download the lines and stations from [ArcGIS Hub](https://hub.arcgis.com/maps/mwcog::metro-lines-and-stations-1/about?path=) and load them as sf (simple features) objects:
...
ggplot(stations) +
  ...
  theme_void() +
  labs(
    title = "Metro Station Graph Represented as a SF Object"
  )
```
...
For the metro, we have points & lines, not polygons. So, to construct our connectivity matrix `M`, we need to use spdep::knn2nb & spdep::knearneigh(), plus some manual adjustment.
//naiive attempt, better attempt below:
//```{r}
//#First, we map the stations that are connected to each other, using KNN w/ k=2.
//#This is probably not the best way to do this: some stations have 2+ neighbors (metro center) & others have only 1 (ends of lines.)
//#Also, there's no way to guarentee that KNN connects the correct stations, since it's going off of distance, not the acutal lines.
//#TLDR: fix somehow... best way I've found so far is manual correction :/

M1 <- spdep::knearneigh(stations, k=2, longlat = TRUE) #knn object

M2 <- knn2nb(M1) #neighbors list generated from knn object

M3 <- spdep::nb2mat(M2, style = "B", zero.policy = TRUE) #connectivity matrix generated from neighbors list

icar.data_M <- prep_icar_data(M3) #ICAR data generated from connectivity matrix
//```

```{r}
# Above is naiive attempt, here is the better version:
# Modified & used this guy's repo to get an adjacency matrix as a csv from WMATA's API: https://github.com/kevcraig/DC-Metro-Analysis/blob/main/Get%20Metro%20Data.py
# Also made a second simple csv with the numeric encodings for the stations
# TY to Sarah (RailMe)

# Read that csv in as a matrix in using as.matrix
adj <- as.matrix(read_csv("RawData/railme/adj.csv", col_names=FALSE))

# Also have a csv with the stations & their associated row/col nums
...
#also need to correct stations order b.c. it's different than in the adjacency matrix which is built from nodes
...
Check convergence ($\hat{R} \approx 1$):
...
We can see that three of the `phi_i` are zero: //that's the case for the census example, not sure hoe to interpret it for this metro version
...
And the marginal variances of the ICAR prior exhibit spatial autocorrelation, with a predictable pattern of higher variance on the edges of the system and lower variance closer to the center of the metro:

```{r}
## drop states with no neighbors
## don't need to do this bc all stations have at least 1 neighbor
...
ggplot(stations3) +
  geom_sf(aes(color=log(phi.var))) +
  scale_color_gradient(
    low = "white",
    high = "darkred"
  ) +
  labs(
    title = "Log Scale Variances of Phi"
  )
```
...
phi1 <- ggplot(stations3) +
  geom_sf(aes(color = phi[1,])) +
  scale_color_gradient2() +
  labs(
    title = "Autocorrelation Pattern for Prior 1"
  )
```

```{r}
phi2 <- ggplot(stations3) +
  geom_sf(aes(color = phi[20,])) +
  scale_color_gradient2() +
  labs(
    title = "Autocorrelation Pattern for Prior 20"
  )
```

```{r}

phi3 <- ggplot(stations3) +
  geom_sf(aes(color = phi[400,])) +
  scale_color_gradient2()+
  labs(
    title = "Autocorrelation Pattern for Prior 400"
  )

phi4 <- ggplot(stations3) +
  geom_sf(aes(color = phi[700,])) +
  scale_color_gradient2() +
  labs(
    title = "Autocorrelation Pattern for Prior 700"
  )
```

```{r}
library(gridExtra)
grid.arrange(phi1, phi2, phi3, phi4, ncol = 2)
```
...
Donegan, Connor. Flexible Functions for ICAR, BYM, and BYM2 Models in Stan. Code Repository. 2021. Available online:
